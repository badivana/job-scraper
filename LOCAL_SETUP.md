...
This document provides detailed instructions for setting up a local development environment for the AI-Powered Job Finder project.
...
2. **(Optional) Create a `.env` file**  
   ...
   The default `.env` already contains sensible defaults for local development. You normally do **not** need to edit it for a first run.
...
A: Yes. You can start the container with an overridden command, e.g.:
   ...
   Or define a separate service in `docker-compose.yml` that uses the same image but a different command.
